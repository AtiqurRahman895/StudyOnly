...
const Register = () => {
    ...
    const {loginWithGoogle,logoutUser, creatUser,updateUserProfile}=useContext(AuthContext)
    ...
    const handleGoogleLoginBtn=async()=>{
        try {
            ...
            if(res?.data?.insertedId){
                logoutUser()
                toast.success(`You have successfully registered. Now, please login again!`)
            }else{
                navigate("/")
                toast.success(`Login successful! Welcome, ${result.user.displayName}!`)
            }
            ...
            <div className="container hero flex items-center justify-center">
                ...
                <div className="fromWrapper max-w-lg">
                    ...
                    <form onSubmit={CreatUserOnSubmit} className="card-body p-0">
                        ...
                        {passwordError && 
                            <label htmlFor="password" className="label">
                                <p className="-alt text-red-500">Password must Be 6 to 20 characters long, Include at least one digit (0-9), one lowercase letter (a-z), one uppercase letter (A-Z) and one special character (@#$%^&*!)</p>
                            </label>
                        }
                        ...
                        <span className="text-center mt-4">
                            Already have an account? <Link to={"/login"} className="link link-hover text-custom-primary hover:font-bold "> <b>Login now</b> </Link>
                        </span>
