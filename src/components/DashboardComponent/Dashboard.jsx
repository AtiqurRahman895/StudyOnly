...
const Dashboard = () => {
    ...
    const COLORS = ['#7c3ff2', '#4580ff', '#43caff', '#b33ff2'];
    ...
    if (isError) {
        ...
        return <p className="text-red-500 text-center">Error fetching data. Please try again.</p>;
    }
                ...
                <div className="place-items-center space-y-10 pt-12">
                  <div className="stats stats-vertical xs:stats-horizontal shadow bg-white">
                    <div className="stat place-items-center">
                      ...
                      <div className="stat-value text-custom-primary">{userQuery.data?.totalUsers}</div>
                    </div>
                    <div className="stat place-items-center">
                      <div className="stat-value text-black !order-2">{sessionQuery.data?.sessionCount}</div>
                      <div className="stat-title text-custom-primary">Total Sessions</div>
                    </div>
                    <div className="stat place-items-center">
                      <div className="stat-title text-black">Total Materials</div>
                      <div className="stat-value text-custom-primary">{sessionQuery.data?.materialCount}</div>
                    </div>
                  </div>
