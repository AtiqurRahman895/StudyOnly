...
const ImageInputSection = ({image,setImage}) => {
    ...
    return (
        <section className="">
            <div className={`max-w-[1536px] mx-auto relative ${image?"aspect-auto":"bg-custom-primary aspect-[4/2.5] sm:aspect-[4/1.5]"}`}>
                <img src={image?image:""} alt="" className="bg-custom-primary max-w-full m-auto" />

                {/* Image */}
                <div className={`${image?"!text-custom-primary":"!text-white "} input-box absolute grid justify-items-center content-center top-0 bottom-0 right-0 left-0`}>
                    ...
                    <div className="input-field label form-control h-fit absolute focus-within:relative scale-0 focus-within:scale-100">
                        ...
                        <input id="image" type="file" accept="image/*" onChange={handleImageChange} className="file-input file-input-ghost bg-transparent focus:!bg-transparent !outline-none file-input-bordered !border-white rounded-sm absolute opacity-0 cursor-pointer"/>
