...
const ApproveModal = ({
  ...
  const [paidInputChecked, setPaidInputChecked] = useState(
    registration_fee > 0
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await secureAxios.put(`/approveOrChangeFee/${_id}`, {
        status,
        registration_fee: finalFee,
      });
      toast.success(
        status === "Pending"
          ? "Successfully approved this session!"
          : "Registration fee changed!"
      );
      setOpenModal(false);
      e.target.reset();
      refetch();
      ...
      console.error(
        status === "Pending"
          ...
          : "Failed to change registration fee!",
        error
      );
      toast.error(
        status === "Pending"
  ...
  return (
    <>
      ...
      >
        {status === "Pending" ? "Approve" : "Change fee"}
      </button>
      <dialog
        id="approve_modal"
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="form-control ">
              <label htmlFor="reason" className="label w-fit text-white">
                <span className="">
                  Do you want this session to be free or paid?
                </span>
              </label>
              <div className="form-control flex-1">
                <label className="label cursor-pointer has-[input:checked]:text-blue-500">
                  ...
                  <input
                    ...
                    value={0}
                    defaultChecked={registration_fee <= 0}
                    type="radio"
                    name="session_fee"
              ...
              <div className="form-control">
                <label className="label cursor-pointer has-[input:checked]:text-custom-primary">
                  ...
                  <input
                    onClick={handlePaidInput}
                    defaultChecked={paidInputChecked}
                    type="radio"
                    name="session_fee"
                    className="radio checked:bg-custom-primary"
                  />
                </label>
              </div>
            </div>
