...
const AllNotes = () => {
  ...
  const handleDeleteButton = async (_id) => {
    const deleteNote = window.confirm(`Are you sure about deleting this note?`);
    if (deleteNote) {
      try {
          ...
          <div className="container space-y-4">
            ...
            <Masonry columnsCount={columnsCount} gutter="20px">
              {notes.map((note, index) => (
                <div
                  key={index}
                  className="bg-custom-primary p-4 lg:p-6 space-y-1 rounded-md w-full"
                >
                  ...
                  <div className="flex justify-center gap-3 pt-4">
                    <Link
                      to={`/dashboard/update_note/${note._id}`}
                    ...
                    <Tooltip anchorSelect=".updateNote" className="">
                      {" "}
                      Update this note{" "}
                    </Tooltip>
                    ...
                    <Tooltip anchorSelect=".deleteNote" className="">
                      {" "}
                      Delete this note{" "}
                    </Tooltip>
                  </div>
